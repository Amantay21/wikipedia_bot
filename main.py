...
def summa(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "Неправильный ввод, введите пожалуйста цифры")
        bot.register_next_step_handler(message, summa)
        return
    if amount > 0:
        ...
        bot.send_message(message.chat.id, "Число должно быть больше нуля , введите пожалуйста цифры")
        bot.register_next_step_handler(message, summa)
...
def callback(call):
    if call.data != 'else':
        ...
        res = curr_conv.convert(amount, values[0], values[1])
...
def my_currency(message):
    try:
        ...
        bot.send_message(message.chat.id, "Что-то пошло не так! Впишите пожалуйста значение заново")
